...
function OutsourcedTab() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('Salvo!'); setModalOpen(false); load()
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500 dark:text-gray-400">{workers.length} terceirizado(s) cadastrado(s)</p>
...
function ProvidersTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500 dark:text-gray-400">{providers.length} prestador(es) cadastrado(s)</p>
        <button onClick={openNew} className="btn-primary btn-sm"><Plus size={14} /> Novo</button>
      </div>
      <div className="table-container">
        <table className="table">
            ...
            {loading
              ...
              : providers.map(p => (
                <tr key={p.id}>
                  ...
                  <td>
                    {p.image_id
                      ? <span className="text-blue-500 text-xs flex items-center gap-1"><Camera size={12} />Cadastrado</span>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Prestador' : 'Novo Prestador'}
        footer={<><button onClick={() => setModalOpen(false)} className="btn-secondary">Cancelar</button><button onClick={handleSubmit} className="btn-primary">Salvar</button></>}>
        <div className="space-y-4">
          <div className="form-group"><label className="label">Nome *</label>
            <input className="input" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
          ...
          <div className="form-group">
            ...
            {(photoPreview || editing?.image_id) && !photoPreview && <p className="text-xs text-green-600 dark:text-green-400 mt-1">✓ Documento já cadastrado</p>}
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm">Terceirizados e prestadores recorrentes</p>
      </div>
      <div className="flex gap-2 border-b border-gray-200 dark:border-gray-700">
        {[['outsourced', UserCog, 'Terceirizados'], ['providers', Truck, 'Prestadores Recorrentes']].map(([t, Icon, label]) => (
